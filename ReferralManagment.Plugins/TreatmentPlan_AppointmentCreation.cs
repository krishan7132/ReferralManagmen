...

{
...

    {
...

            {
...

                {
...

                    {
                        ...
                        Entity _contact = _service.Retrieve("contact", contact.Id, new ColumnSet("fullname"));
                          ...
                          <entity name='clrcomp_authorization'>
...

                            {
...

                                {
                                    ...
                                    if (_planstartingdateTimeLocal <= _authorizationStartDatetimeLocal) { _planstartingdateTimeLocal = _authorizationStartDatetimeLocal; }



                                    List<DateTime> _appointmentsDate = GetDatesBetween(_planstartingdateTimeLocal, _authorizationendDatetimeLocal, _daysofWeek, _authorizationVisitsLeft, tracingService, _service,context);
...

                                    {
                                        ...
                                        Appointment["subject"] = "Auto Created Appointment for " + _contact.GetAttributeValue<string>("fullname");

                                        Appointment["regardingobjectid"] = new EntityReference("clrcomp_treatmentplan", _treatmentPlan.Id);

                                        Appointment["scheduledstart"] = date;

                                        Appointment["scheduledend"] = date.AddMinutes(60);

                                        Appointment["clrcomp_appointmenttype"] = new OptionSetValue(397780001);

                                        Entity party1 = new Entity("activityparty");

                                        party1["partyid"] = new EntityReference("contact", contact.Id);

                                        EntityCollection required = new EntityCollection();

                                        required.Entities.Add(party1);

                                        Appointment["requiredattendees"] = required;

                                        Appointment["clrcomp_facility"] = _treatmentPlan.GetAttributeValue<EntityReference>("clrcomp_facility");
        ...
        public List<DateTime> GetDatesBetween(DateTime _startdate, DateTime _endDate, List<string> selecteddays, int visits, ITracingService tracingService, IOrganizationService service, IPluginExecutionContext context)

        {
            ...
            foreach (var closures in businessClosures)

            {

                var startDate = ((DateTime)closures["effectiveintervalstart"]).Date;

                var endDate = ((DateTime)closures["effectiveintervalend"]).Date;

                businessClosureDate.Add(startDate);

                tracingService.Trace("Business Closure Date : " + (startDate));
...

        {
            ...
            Entity org = service.Retrieve("organization", context.OrganizationId, new ColumnSet("businessclosurecalendarid"));
